        ...
        <div class="manip-header" style="margin-left: 10px">
            ...
            <el-row>
                <el-col>
                    ...
                    >添加</el-button
                    >
                    <el-button @click="myact" type="primary" size="small" round
                    >我的活动</el-button
                    >
                </el-col>
            </el-row>
...
<script>
    export default {
        ...
        created(){
            const _this = this
            axios.get('http://localhost:8181/activity/findAll/1/3').then(function (resp) {
                console.log(resp)
        ...
        methods:{
            Search(){
                ...
                axios.get('http://localhost:8181/activity/findAll/1/3/'+_this.formInline.range+'/'
            ...
            page(currentPage){
                const _this = this
                axios.get('http://localhost:8181/activity/findAll/'+currentPage+'/3').then(function (resp) {
                    // console.log(resp)
                    _this.tableData = resp.data.content
                    _this.total = resp.data.totalElements
                })
            ...
            look(row){
                if(row.state=="报名中"){
                    const _this = this;
                    const _uid = JSON.parse(window.localStorage.getItem('access-admin')).id;
            ...
            myact(){
                this.$router.replace({
                    path:'/myact',
                    query:{
                        id:JSON.parse(window.localStorage.getItem('access-admin')).id,
                    }
                })
            },
            back(){
            ...
            Submit(){
                const _this = this
                const _uname = JSON.parse(window.localStorage.getItem('access-admin')).userName;
